...
ENEMY_IMAGE  = pygame.image.load(os.path.join(dirname, 'mine.png'))
ME_IMAGE = pygame.image.load(os.path.join(dirname, "me.png"))
SEA_IMAGE = pygame.image.load(os.path.join(dirname, "sea.png"))
FLAG_IMAGE = pygame.image.load(os.path.join(dirname, "flag.png"))
...
def my_senzor(me, mines):
    ...
    # get distance to closset mine
    for i, mine in enumerate(mines):

        xDis = abs(xMe - mines[i].rect.x)
        yDis = abs(yMe - mines[i].rect.y)
    ...
    # get quadrant
    if (mines[clossestMine].rect.x - xMe) < 0:
        quadrant[0] = -1
    else:
        quadrant[0] = 1
    if (mines[clossestMine].rect.y - yMe) < 0:
        quadrant[1] = -1
    else:
        quadrant[1] = 1
    
    return [smallestDist, quadrant ]


# -----> ZDE je prostor pro vlastní senzorické funkce !!!!

def wallSenzor(me, WIDTH, HEIGHT):
    xDist = me.rect.x
    yDist = me.rect.y
    
    return [xDist, WIDTH - xDist, yDist, HEIGHT - yDist]
...
def nn_function(inp, wei):
    ...
    # neurons
    hidden.append(np.dot(readyInp, wei[0:7])) 
    hidden.append(np.dot(readyInp, wei[7:14])) 
    hidden.append(np.dot(readyInp, wei[14:21])) 
    hidden.append(np.dot(readyInp, wei[21:28])) 
    
    for i in range(4):
        neurons.append(np.dot(hidden, wei[4*i +28: 4*(i+1) +28])) 
    '''
    
    for i in range(4):
        neurons.append(np.dot(readyInp, wei[3*i: 3*(i+1)])) 
    
    # activation function
    
    out = neurons.index([max(neurons)]) # move in +X, -X, +Y, -Y
...
def handle_mes_movement(mes, mines, flag):
    ...
    for me in mes:
        ...
        if me.alive and not me.won:
            ...
            inp.append(wallSenzor(me, WIDTH, HEIGHT))
...
def handle_mes_fitnesses(mes, flag, WIDTH, HEIGHT, SIMSTEPS):    
    ...
    for me in mes:
        ...
        if me.won:
            result +=300
                ...
                xDist = abs(me.rect.x - flag.rect.x)
                yDist = abs(me.rect.y - flag.rect.y)
                
                dist = xDist + yDist
                
                pctDistFromFlag = dist / maxDist
                
                pctFurthestFromFlag = 1 - pctDistFromFlag
                
                result += np.floor(pctFurthestFromFlag * 200)
            
            
                #result += np.floor((me.timealive / SIMSTEPS) * 50)
...
def main():
    ...
    DELKA_JEDINCE = 12   # <--------- záleží na počtu vah a prahů u neuronů !!!!!
    ...
    while run:  
        ...
        if timer >= SIMSTEPS or nobodys_playing(mes): 
            ...
            handle_mes_fitnesses(mes, flag, WIDTH, HEIGHT, SIMSTEPS)   # <--------- ZDE funkce výpočtu fitness !!!!
