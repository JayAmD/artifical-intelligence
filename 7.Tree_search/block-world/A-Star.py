...
class Env:
    ...
    def path_planner(self, startx, starty):
        ...
        # heap el structure: (heuristic, G-heuristic, H-heuristic, (x, y), parentPositionInClosedQue)
        start = (self.getHHeuristic((startx, starty)), 0, self.getHHeuristic((startx, starty)), (startx, starty), None)
        ...
        while len(openQue) and not isGoal:
            ...
            if currNode[3] == (self.goalx, self.goaly):
                isGoal = True
            
            neighbors = self.getValidNeighbors(currNode[3])

            for neighbor in neighbors:
                if not self.wasVisited(closedQue,neighbor):
                    hHeuristic = self.getHHeuristic(neighbor)
                    parentPosition = len(closedQue)-1
                    gH = currNode[1] + 1
                    fH = gH + hHeuristic
                
                    heapq.heappush(openQue, (fH, gH, hHeuristic, neighbor, parentPosition))
        ...
        if isGoal:
            ...
            while(not isStart):
                ...
                reverseList.append(currNode[3])

                if currNode[4] == None:
                    isStart = True
                else:
                    currNode = closedQue[currNode[4]]
    ...
    def getHHeuristic(self, coord):
        goalX = self.goalx
        goalY = self.goaly
    
        #h = math.sqrt((coord[0] - goalX)**2 + (coord[1] - goalY)**2)
        h = max(abs(coord[0] - goalX), abs(coord[1] - goalY))
        return h
    ...
    def wasVisited(self, que, node ):
        for fH, gH, hH, closedNode, p in que:
            if closedNode == node :
                return True
        return False
