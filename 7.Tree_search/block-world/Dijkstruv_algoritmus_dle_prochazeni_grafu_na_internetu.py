...
class Env:
    ...
    # <------    ZDE vlastní metoda
    def path_planner(self, startx, starty):
        ...
        # heap el structure: (heuristic, (x, y), parentPositionInClosedQue)
        start = (0, (startx, starty), None)
        
        openQue.append(start)
        
        #main loop
        while len(openQue) and not isGoal:
            
            currNode = openQue.pop()
            
            closedQue.append(currNode)

            if currNode[1] == (self.goalx, self.goaly):
                isGoal = True
            
            neighbors = self.getValidNeighbors(currNode[1])

            for neighbor in neighbors:
                wasVsited, index, (oldH, nodeCoord, p) = self.wasVisited(closedQue,neighbor)
                
                heuristic = 1 + currNode[0]
                parentPosition = len(closedQue)-1

                if not wasVsited:
                    for idx, node in enumerate(openQue):
                        if node[1] == neighbor and node[0]<heuristic:
                            heuristic = node[0]
                            parentPosition = node[2]
                            openQue.remove(node)
                            
                    sortingList.append((heuristic, neighbor, parentPosition))
                
                else:

                    if oldH > heuristic:
                        print((oldH, nodeCoord), heuristic)
                        closedQue[index] = (heuristic, nodeCoord, parentPosition)
                
            sortingList = sorted(sortingList, key=lambda x: -x[0])
            openQue.extend(sortingList)
            sortingList.clear()
        ...
        if isGoal:
            ...
            currNode = closedQue.pop()
            
            while(not isStart):
                
                reverseList.append(currNode[1])

                if currNode[2] == None:
                    isStart = True
                else:
                    currNode = closedQue[currNode[2]]
    ...
    def getHeuristic(self, coord):
        goalX = self.goalx
        goalY = self.goaly
        return max(abs(coord[0] - goalX), abs(coord[1] - goalY))
    
    def getValidNeighbors(self, coord):
        validNeighbors = []

        neighbors = self.get_neighbors(coord[0], coord[1])
        for neighbor in neighbors:
            if self.is_empty(neighbor[0], neighbor[1]):
                validNeighbors.append(neighbor)
        return validNeighbors
    
    def wasVisited(self, que, node ):
        for idx, (h, closedNode, p) in enumerate(que):
            if closedNode == node :
                return True, idx, (h, closedNode, p)
...
def main():
    ...
    p, t = env.path_planner(env.startx, env.starty)   # cesta pomocí path_planneru prostředí
    ufo.set_path(p, t)
    # ---------------------------------------------------
